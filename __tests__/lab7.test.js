describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
   ...
   for(let i = 0; i < prodItemsData.length; i++){
    ...
    if (currentValue.title.length == i) { allArePopulated = false; }
    if (currentValue.price.length == i) { allArePopulated = false; }
    if (currentValue.image.length == i) { allArePopulated = false; }
  ...
  it('Checking number of items in cart on screen', async () => {
     ...
     */
      ...
      for (let i = 0; i < prodItemsData.length; i++){
          currentValue = prodItemsData[i];
          const shadRoot = await currentValue.getProperty('shadowRoot');
          const button = await shadRoot.$('button');
          await button.click();
      }
    
      const cartItems = await page.$('#cart-count');
      const cartText = await cartItems.getProperty('textContent');
      const cartCount = await cartText.jsonValue();
